...
def read(topic='workflow'):
    ...
    topics={
        ...
        'proposals':{'media':'動画の区間・画像・音声・文字・composition・GLB/glTFをpresent_referencesで提示履歴へ追加できる。画像と動画など複数形式を一緒に表示でき、スクロールで過去の案を見返し、各実物を拡大できる。公開URLをsourceに渡すと取得・登録・表示をまとめて行う。',
