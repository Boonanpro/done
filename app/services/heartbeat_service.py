    ...
    try:
        ...
        for jsonl_path in jsonl_files:
            try:
                ...
                synced_count += len(new_uuids) - synced_count
            except Exception as e:
...
async def run_heartbeat() -> dict:
    ...
    """
    # コンパクションサマリーをミラーリング（LLM呼び出し前に必ず実行）
    try:
        sync_compaction_summaries()
    except Exception as e:
        logger.warning(f"[compaction_sync] sync error (non-fatal): {e}")
