...
def page():
    return HTMLResponse(PAGE.read_text(encoding='utf-8'), headers={
        ...
        'Content-Security-Policy': "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; connect-src 'self' https://api.openai.com; img-src 'self' data: https:; media-src 'self' blob: https:; frame-src https://www.youtube-nocookie.com https://player.vimeo.com; frame-ancestors 'none'",
    })


@router.get('/script')
def script():
    return Response(PAGE.with_suffix('.js').read_text(encoding='utf-8'), media_type='application/javascript',
...
def proposal_script():
    return Response(PAGE.with_name('editor-proposals.js').read_text(encoding='utf-8-sig'),media_type='application/javascript',headers={'Cache-Control':'no-store'})
...
async def add_reference(request: Request, body: ReferenceInput):
    ...
    try:
        return await asyncio.to_thread(editor_references.resolve, body.room_id, body.content_id, body.source)
...
from app.services.editor_presentation import ITEM_SCHEMA, DESCRIPTION as PRESENT_DESCRIPTION
from app.services import editor_references
...
EDITOR_TOOLS = [t for t in _CHAT_TOOLS if t['name'] in {'timeline_edit', 'timeline_frame', 'read_skill'}] + [
    ...
    function('inspect_range','指定区間と前後の構成、素材情報、元音声の解析を読む。配置や重複の調査に使う。',{'start':{'type':'number'},'end':{'type':'number'}},['start','end']),
    ...
    function('timeline_state','現在付近の編集要素を確認。別の時刻はt、全体構成が必要なときだけfull=true。',
             {'t':{'type':'number'},'full':{'type':'boolean'}},[]),
        ...
        async with _turn_lock(body.room_id,body.turn_id):
            try:
                ...
                if state is None:
                    ...
                    state={'inputs':editor_reasoning.initial_input({**current['context'],'conversation_state':body.runtime},rows['messages']),'pending':[]}
                else:
                ...
                async for event in editor_reasoning.stream_response(state['inputs'],editor_reasoning.tools_for_conversation(EDITOR_TOOLS)):
...
async def _tool(request: Request, body: ToolRequest):
    ...
    if body.name=='search_web_references':
        ...
        return await search_web(args.get('queries'))
    ...
    if body.name=='search_references':
        ...
        return await search(body.room_id,cid,args.get('queries'),current)
    ...
    if body.name=='present_references':
        ...
        return present(body.room_id,cid,args.get('items'))
    if body.name in {'project_status','inspect_range','asset_image','update_work','edit_history','measure_speech'}:
        ...
        try:
            ...
            if body.name == 'inspect_range':return await asyncio.to_thread(project.inspect_range,body.room_id,cid,args['start'],args['end'])
    ...
    if body.name == 'timeline_state':
        result = workflows.compact_state(body.room_id,cid,float(args.get('t',ctx['playhead'])),ctx['selected'],bool(args.get('full',False)))
        result.update(playhead=ctx['playhead'], selected=ctx['selected'], edit_scope=turn['scope'])
        return result
    if body.name == 'timeline_frame':
        return await asyncio.to_thread(tl.render_frame_b64, body.room_id, cid, float(args.get('t', turn.get('verify_at', ctx['playhead']))))
    ...
    if body.name == 'read_skill':
        ...
        name = args.get('name')
        if name:
            return {'text': SkillRegistry.get_prompt(name)[:22000]}
        return {'skills': [{'name': s.name, 'description': s.description} for s in SkillRegistry.list_all()]}
    if body.name not in {'timeline_edit', 'delegate_edit','edit_captions','batch_edit','add_music','resize_captions'}:
    ...
    if not preparing and ctx.get('input_mode') == 'voice' and turn['scope'] is not None and not turn.get('target_bound'):
        ids = []
        if body.name in {'resize_captions','edit_captions','delegate_edit'}:
            ids = args.get('clip_ids', [])
    ...
    if ctx.get('input_mode')=='voice' and turn['scope'] is not None and body.name in {'batch_edit','timeline_edit'}:
        ...
        if spans:
            ...
            turn['target_bound']=True
    if not preparing and turn['scope'] is not None and not turn['scope']['clip_ids'] and not turn['scope'].get('spans'):
        return {'ok': False, 'error': '音声では明確な対象をresolve_targetで結んでください。曖昧なときだけpropose_targetで確認。テキストでは範囲を選択してください'}
    if body.name == 'delegate_edit':
        from fastapi import BackgroundTasks
        from app.api.production_asset_routes import CreateJobRequest, create_job
        _, seq = tl.live_sequence(body.room_id, cid)
        if not preparing and td.sequence_hash(seq) != turn['sequence_hash']:
        ...
        job = await create_job(CreateJobRequest(room_id=body.room_id, content_id=cid, instruction={
            ...
            'selected_clips': ctx['selected'] if not preparing and turn['scope'] is not None else [], 'content_id': cid,
            'editor_guarded': True, 'editor_expected_hash': None if preparing else turn['sequence_hash'],
