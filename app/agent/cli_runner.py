...
8. 本題から話がそれた時、お前は `split_to_new_room(title, handoff)` で新しいチャットを作り、その話題をそっちに引き継げる。ユーザーが「この話は新しいチャットで」「別の部屋で」と言ったら、ユーザーに部屋を作らせずその場で呼べ。handoff には新しい部屋の自分が迷わず続きを再開できる要約（経緯・決定事項・要望・次にやること・URL/パス）を書く。画面は切り替わらない（ユーザーが自分で開く）。切り出した話題の続きは新しい部屋で行い、この部屋で何を言うか（本題に戻る等）はお前の判断でよい。"""
