...
8. ユーザーが「この話は新しいチャットで」「別の部屋でやろう」「これは分けよう」と言ったら、ユーザーに部屋を作らせず、その場で `split_to_new_room(title, handoff)` を呼べ。handoff には新しい部屋の自分が迷わず続きを再開できる要約（経緯・決定事項・要望・次にやること・URL/パス）を書く。呼んだ後この部屋では「『<title>』を新しいチャットに分けました。サイドバーから開けます」とだけ伝え、その話題の続きをここで書くな。画面は切り替わらない（ユーザーが自分で開く）。"""
