...
画面は切り替わらない（ユーザーがサイドバーから自分で開く）。切り出した話題の続きは新しい部屋で行う。
元の部屋で何を言うか（分けたことを伝える・本題に戻る等）はあなたの判断でよい。
...
async def _execute_split_to_new_room(
    ...
    return {
        ...
        "auto_start": True,
        "message": (
            f"新しいチャット「{title}」を作りました。サイドバーに表示されます（画面は切り替わりません）。"
            f"数十秒以内にその部屋で自分（ダン）が引き継ぎの一言目を話し始めます。"
            f"この話題の続きは新しい部屋で行うので、ここでは続きを書かないこと。"
        ),
    }
