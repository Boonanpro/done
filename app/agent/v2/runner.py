...
class AgentRunner:
    ...
    def _is_project_chat_room(self) -> bool:
        """現在のセッションがプロジェクトチャットかどうかを返す（キャッシュ付き）"""
        if hasattr(self, "_is_project_chat_cached"):
        ...
        try:
            ...
            self._is_project_chat_cached = bool(result.data)
        except Exception:
            self._is_project_chat_cached = False
        return bool(self._is_project_chat_cached)
