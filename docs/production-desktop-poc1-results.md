...
# PoC‑2 続報（2026-06-28）= 合成・追従は成立。一体化はMVPでDComp。

実機検証（スクショ＋ユーザー目視）で確定した事実：

## ✅ 成立（premise クリア）
- ネイティブGES映像がWebView2のプレビュー枠の位置に**実際に合成表示**される。
- ウィンドウの**移動・リサイズに追従**する（ユーザー確認済み）。

## 実装の要点（ハマりと解法）
- ❌ `d3d11videosink` に外部作成HWNDを `set_window_handle` すると**黒画面**（音は出るが描画されない）。`STATIC`クラスだと白＋クラス名テキストが描画される。
- ✅ 解法：**d3d11videosink に自前ウィンドウ（クラス `GSTD3D11`）を作らせ**、Rustシェルが `FindWindowW("GSTD3D11")` で発見→ボーダー除去（`WS_POPUP`化）→**最前面で青枠の画面座標に `SetWindowPos` 追従**。映像は別プロセス(player)の自前ウィンドウなのでスワップチェーンが正しく動く。
- 自己検証は **ImageGrab スクショ**で実施（`videotestsrc` のバーが枠を埋めるのを確認＝合成・配置OK）。

## ⚠️ 既知の限界（ユーザー観測）= §6①オーバーレイ面のトレードオフ
動画が**独立した常時最前面ウィンドウ**のため：タスクビューで本体と分離／無関係の別ウィンドウが動画と枠の“間”を通れる／枠にクリップされない。＝「一体化アプリ」でなく「枠の上に貼り付け」に見える。

## → 真の一体化はMVPで「方式②」へ
**DirectComposition / WebView2 ビジュアルホスティング**（`CoreWebView2CompositionController`）で、映像を **WebView2のビジュアルツリー内のDCompビジュアル**として合成する。これで1ウィンドウ・正しい重なり順・クリップ・タスクビュー一体化が実現。これはMVPの実装項目（§6②「GPUテクスチャ共有」）。

## premise 進捗（最終）
- ✅ PoC‑1（エンジン素性）/ ✅ PoC‑3（編集即時）/ ✅ **PoC‑2（合成・追従）成立**。
- → **3つ green。MVPへ**（MVP初期にDComp一体化＋デコーダプール最適化）。
