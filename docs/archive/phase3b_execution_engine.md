...
## Smart Fallback（スマート代替提案機能）

**追加日**: 2024年12月26日
**更新日**: 2024年12月27日 - LLMベースの汎用フォールバックに改善

### 概要

タスク実行が失敗した場合に、**LLMを使って状況に応じた最適な代替案を提案**する機能。
単純な「バスがだめなら電車」ではなく、距離・コスト・利便性を考慮してランク付けした提案を行う。

### 対応タスクタイプ

| タスクタイプ | フォールバック内容 |
|------------|------------------|
| TRAVEL | 距離に応じた最適な移動手段（近距離→タクシー/電車、長距離→新幹線/飛行機） |
| PURCHASE | 別のECサイト、類似商品、実店舗での購入 |
| その他 | 目的達成のための創造的な代替案 |
...
- `app/agent/agent.py` の `_generate_fallback_proposals()` メソッド
- `execute_task()` 内で実行失敗時に自動呼び出し
- **全タスクタイプで動作**（TRAVELに限らない）

### テスト

```bash
pytest tests/test_execution_engine.py::TestSmartFallback -v
```

---

## Revise with Re-search（訂正時の再検索機能）

**追加日**: 2024年12月27日

### 概要

ユーザーが訂正リクエスト（「鳥取市までで良いからバスか電車で探して」など）を送った際に、
**単にLLMで提案を修正するのではなく、実際に再検索して新しい結果を基に提案を生成**する機能。
...
2. 実行失敗 + 代替案表示
   ↓
3. ユーザー訂正: 「鳥取市までで良いからバスか電車で探して」
   → POST /api/v1/task/{id}/revise
   ↓
4. [NEW] 願望をマージ: 「大阪から鳥取市へバスか電車で」
   ↓
5. [NEW] 再検索: search_bus(), search_train()
   ↓
6. [NEW] 検索結果を基に新しい提案を生成
   ↓
7. 承認待ち（ユーザーがconfirm）
   ↓
8. 実行
...
- `app/agent/agent.py` の `revise_task()` メソッド

### 改善点

| Before | After |
|--------|-------|
| LLMの推測で提案を修正 | 再検索して実データで提案 |
| 古い検索結果のまま | 新しい検索結果を取得 |
| 目的地変更が反映されにくい | 正確に反映される |
