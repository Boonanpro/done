...
async def test_amazon_search():
    """テストケース2: Amazon商品検索"""
    print("=" * 60)
    print("テストケース2: Amazonで「アベンヌウォーター 50ml」を検索")
    print("=" * 60)

    # レコーダーとエージェントを作成
    recorder = BrowserRecorder(user_id="test_user")
    agent = VisualAgent(recorder=recorder, max_steps=15)

    # タスクを実行
    result = await agent.execute_task(
        task="Amazonで「アベンヌウォーター 50ml」を検索して、商品一覧を表示する",
        site="amazon.co.jp",
...
if __name__ == "__main__":
    result = asyncio.run(test_amazon_search())
