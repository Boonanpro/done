    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if search_result.options:
                print("\n列車候補:")
                for option in search_result.options[:3]:
            ...
            # ★★★ 座席選択（2人分・隣り合う席） ★★★
            print("💺 座席選択中（2人分・隣り合う席）...")
            print("  商品: 最初の商品（通常は最安値のスマートEX）")
            print("  座席位置: 通路側C（2人分なので隣り合う2席が自動割り当て）")
            print("  席が離れても良い: False（隣り合う席を希望）")

            seat_result = await complete_seat_selection(
                page,
                product_index=0,
                seat_position="通路側C",
                allow_separate_seats=False,  # 隣り合う席を希望
            )
            ...
            print("=" * 70)
            print("✅ 複数人予約テスト完了（2人・隣り合う席）")
            print("=" * 70)
            print()
            print("確認画面で以下を確認してください:")
            print("  - 大人2名分の予約になっているか")
            print("  - 隣り合う2席が割り当てられているか")
            print("  - 料金が2人分になっているか")
            print()
            ...
            import traceback
            traceback.print_exc()
            await page.screenshot(path="test_multi_error.png")
        finally:
            await browser.close()
