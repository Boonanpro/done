...
room = 'assistant-create-test-' + uuid.uuid4().hex[:8]
...
with httpx.Client(headers={'Authorization': 'Bearer '+token}, timeout=30) as client:
    ...
    assert tool('save_brief', {'intent': '新規制作の経路を検証する', 'taste': '簡潔な白い文字と黒い背景'})['ok']
    job = tool('delegate_edit', {'instruction': '動作確認用に3秒の無音動画の編集可能な下書きを作ってください。0〜3秒に白い字幕「制作のテスト」を1つ置くだけです。add_captionとvalidate_draftとrender_frameで完了できます。音声・画像・動画生成やWeb調査は不要です。確認後に終了してください。'})
    ...
    assert status['status'] == 'done' and '制作のテスト' in texts, (status.get('error'), texts)
