"""Supabase Management API 経由で 057_chat_artifact_room_scope.sql を適用する。"""
import json
import sys
...
MIGRATION_FILE = PROJECT_ROOT / "supabase" / "migrations" / "057_chat_artifact_room_scope.sql"
...
def get_access_token() -> str:
    with sync_playwright() as p:
        ...
        if "/sign-in" in page.url:
            print("[ERROR] 未ログイン。先に Playwright persistent context で GitHub OAuth を済ませてください")
            ctx.close()
            sys.exit(1)
...
def main():
    ...
    print("\n[STEP] 057_chat_artifact_room_scope.sql 適用")
