...
result=run_timeline_agent(room_id=r,user_id='2582a188-ff24-4a4f-b989-6063034d90b2',content_id=cid,job_id=job,
    ...
    instruction=f"隔離した制作テストです。字幕 {caption['id']} の文言だけを『朝の一杯を。』へ変えてください。位置・尺・サイズなど他の属性と他クリップは保持。素材生成や外部サービス操作は不要です。timelineツールで編集・検証し、実行結果を一文で報告してください。")
...
next(c for t in expected['tracks'] for c in t['clips'] if c['id']==caption['id'])['text']='朝の一杯を。'
