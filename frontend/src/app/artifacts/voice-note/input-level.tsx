...
export function InputLevel({ stream }: { stream: MediaStream | null }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      return () => { active=false;window.removeEventListener('pointerdown',resume);window.removeEventListener('keydown',resume);cancelAnimationFrame(frame);source.disconnect();analyser.disconnect();void context?.close(); };
  ...
  return <div style={{margin:'12px 0'}}>
    ...
    <small>{unavailable?'入力レベルを表示できません。':stream?'入力を確認中':'マイクの接続を待っています。'}</small>
  </div>;
}
