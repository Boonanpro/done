...
export function PageBody() {
 ...
 useEffect(()=>{if(typing||!dirty||!selected||busy||editing||saving||saveError)return;const timer=setTimeout(()=>{void save().catch(()=>{});},500);return()=>clearTimeout(timer);},[draft,dirty,selected?.id,busy,editing,saving,saveError,typing]);
 ...
 useEffect(()=>{if(!audioBlob||!backupReady||recording||busy||editing||attemptedAudio.current===audioBlob)return;attemptedAudio.current=audioBlob;void run(async()=>{setMessage('録音を自動保存しています…');await persistAudio(new File([audioBlob],'録音.'+(audioBlob.type.includes('mp4')?'m4a':'webm'),{type:audioBlob.type}));setMessage('録音を自動保存しました。');});},[audioBlob,backupReady,recording,busy,editing,retryAudio]);
 ...
 <div className={styles.editorTop}><h2>記事の仕上がり</h2><div className={styles.actions}><button data-edit-id="voice-note-x-jump" onClick={()=>document.getElementById("voice-note-x-drafts")?.scrollIntoView({block:"start"})}>Xで届ける</button>{mediaUndo&&<button type="button" aria-label="変更を取り消す" title="変更を取り消す" onClick={()=>{update(mediaUndo);setMediaUndo(null);}}><Undo2 size={17}/></button>}</div></div>
 <InlinePaper key={'paper-'+selected.id} ref={paper} free={draft.free} paid={draft.paid} layout={draft.editorLayout} onActivity={setTyping} disabled={busy||editing||recording} onDelete={removeImage} onSelection={setSelection} onImage={src=>{setImageTarget(src);setTimeout(()=>document.querySelector('[aria-label="AI編集"]')?.scrollIntoView({block:'center',behavior:'smooth'}),0);}} onMove={(free,paid,html)=>update({free,paid,editorLayout:{free,paid,html},status:'編集中'})}/>
 <AIPanel requestError={error} key={'ai-'+selected.id} article={draft} selection={selection} imageTarget={imageTarget} onTarget={setImageTarget} paper={paper} disabled={busy||editing||recording} onApply={update} onRun={async(instruction,intent,chosen,imageUrl)=>{paper.current?.freezeSelection();await run(async()=>{setMediaUndo({free:draft.free,paid:draft.paid,images:draft.images});const row=await save();const token=(await import('@/stores/auth-store')).useAuthStore.getState().token;const r=await fetch('/api/voice-note/editorial',{method:'POST',headers:{'Content-Type':'application/json',...(token?{Authorization:`Bearer ${token}`}:{})},body:JSON.stringify({id:row.id,mode:'assistant',instruction,intent,selection:chosen,imageUrl})});const data=await r.json();if(!r.ok)throw new Error(data.error||'AIを開始できませんでした。');setSelected(data.row);setDraft(articleOf(data.row));setRows(v=>v.map(x=>x.id===row.id?data.row:x));setDirty(false);});}}/>
