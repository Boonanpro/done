...
import { Mic, Square, Upload, Plus, FileText, ArrowRight, RefreshCw, LockKeyhole, Send, Trash2, Undo2, Pencil, Eye } from 'lucide-react';
...
export function PageBody() {
 ...
 useEffect(()=>{setMediaUndo(null);},[selected?.id]);
 ...
 <div className={styles.editorTop}><h2>記事の仕上がり</h2><button aria-label={preview?'本文を編集':'本文編集を閉じる'} title={preview?'本文を編集':'本文編集を閉じる'} onClick={()=>setPreview(!preview)}>{preview?<Pencil size={17}/>:<Eye size={17}/>}</button></div>
 {mediaUndo&&<button type="button" aria-label="画像の削除を取り消す" title="画像の削除を取り消す" onClick={()=>{update(mediaUndo);setMediaUndo(null);}}><Undo2 size={17}/></button>}
 <ArticlePaper free={draft.free} paid={draft.paid} price={draft.price} disabled={busy||editing||recording} onDelete={removeImage} onMove={(free,paid)=>update({free,paid,status:'編集中'})}/>
 {!preview&&<><label className={styles.field}>無料部分<textarea rows={10} value={draft.free} onChange={e=>update({free:e.target.value,status:'編集中'})}/></label><label className={styles.field}>有料部分<textarea rows={10} value={draft.paid} onChange={e=>update({paid:e.target.value,status:'編集中'})}/></label></>}
 ...
 <div className={styles.footer}><button aria-label="記事を削除" title="記事を削除" onClick={()=>requestDelete(selected)} disabled={busy||editing||recording}><Trash2 size={17}/></button><button className={styles.primary} disabled={busy||editing||recording||Boolean(audioBlob)||!draft.free.trim()||draft.questions.length>0} onClick={()=>setConfirm('publish')}><Send size={16}/>投稿内容を確認</button></div></fieldset>}</div>}
