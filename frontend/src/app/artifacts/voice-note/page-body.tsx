...
import { readLocation, writeLocation } from './navigation';
...
export function PageBody() {
 ...
 <div className={styles.editorTop}><h2>記事の仕上がり</h2><div className={styles.actions}><button data-edit-id="voice-note-x-jump" onClick={()=>{const url=new URL(location.href);url.hash="voice-note-x-drafts";history.pushState(null,"",url);document.getElementById("voice-note-x-drafts")?.scrollIntoView({block:"start"});}}>Xで届ける</button>{mediaUndo&&<button type="button" aria-label="変更を取り消す" title="変更を取り消す" onClick={()=>{update(mediaUndo);setMediaUndo(null);}}><Undo2 size={17}/></button>}</div></div>
