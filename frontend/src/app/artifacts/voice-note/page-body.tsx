...
import { Mic, Square, Upload, Plus, FileText, ArrowRight, RefreshCw, LockKeyhole, Send, Trash2, Undo2, Bold, Italic, Pilcrow, Heading2, Heading3 } from 'lucide-react';
...
export function PageBody() {
 ...
 <div className={styles.editorTop}><h2>記事の仕上がり</h2><div className={styles.actions}>{([{tag:"p",label:"本文",Icon:Pilcrow},{tag:"h2",label:"見出し",Icon:Heading2},{tag:"h3",label:"小見出し",Icon:Heading3}]).map(({tag,label,Icon})=><button key={tag} type="button" aria-label={label} title={label} onMouseDown={e=>e.preventDefault()} onClick={()=>paper.current?.format("formatBlock",tag)}><Icon size={17}/></button>)}<button type="button" aria-label="太字" title="太字" onMouseDown={e=>e.preventDefault()} onClick={()=>paper.current?.format('bold')}><Bold size={17}/></button><button type="button" aria-label="斜体" title="斜体" onMouseDown={e=>e.preventDefault()} onClick={()=>paper.current?.format('italic')}><Italic size={17}/></button>{mediaUndo&&<button type="button" aria-label="変更を取り消す" title="変更を取り消す" onClick={()=>{update(mediaUndo);setMediaUndo(null);}}><Undo2 size={17}/></button>}</div></div>
 <InlinePaper key={'paper-'+selected.id} ref={paper} free={draft.free} paid={draft.paid} disabled={busy||editing||recording} onDelete={removeImage} onSelection={setSelection} onImage={src=>{setImageTarget(src);setTimeout(()=>document.querySelector('[aria-label="AI編集"]')?.scrollIntoView({block:'center',behavior:'smooth'}),0);}} onMove={(free,paid)=>update({free,paid,status:'編集中'})}/>
