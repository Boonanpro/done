...
import { articleOf, publishRequest, emptyArticle, ROOM_ID, type Row, type Article } from './model';
...
export function PageBody() {
 ...
 const [recording,setRecording]=useState(false);const [confirm,setConfirm]=useState<'publish'|'delete'|null>(null);
 ...
 async function run(fn:()=>Promise<void>){if(busy||editing)return;setBusy(true);setError('');setMessage('');try{await fn();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 ...
 {tab==='つくる'&&<div className={styles.workspace}><aside className={styles.side}><button className={styles.primary} disabled={busy||editing||recording||Boolean(audioBlob)} onClick={()=>void create()}><Plus size={17}/>新しい記事</button><small>記事の素材と下書き</small>{rows.map(r=><button className={selected?.id===r.id?styles.chosen:styles.item} key={r.id} onClick={()=>void pick(r)} disabled={busy||editing}><span>{articleOf(r).title}</span><small>{articleOf(r).status}</small></button>)}<div className={styles.hint}><strong>こんな話から</strong><p>最近、面倒だったこと。<br/>試して失敗したこと。<br/>前より楽になった工夫。</p></div></aside>
 ...
 <div className={styles.footer}><button onClick={()=>setConfirm('delete')} disabled={busy||editing||recording}>削除</button><button className={styles.primary} disabled={busy||editing||recording||Boolean(audioBlob)||!draft.free.trim()||draft.questions.length>0} onClick={()=>setConfirm('publish')}><Send size={16}/>投稿内容を確認</button></div></fieldset>}</div>}
 {tab==='記事一覧'&&<section className={styles.list}><h2>すべての記事 <small>{rows.length}本</small></h2>{!rows.length?<p>まだ記事はありません。「つくる」から音声を追加できます。</p>:rows.map(r=>{const a=articleOf(r);return <button key={r.id} className={styles.listRow} onClick={()=>void pick(r)}><FileText size={20}/><span><strong>{a.title}</strong><small>{a.status} · {a.price?'¥'+a.price.toLocaleString():'無料'}{a.scheduledAt?' · 公開希望 '+a.scheduledAt.replace('T',' '):''}</small></span><ArrowRight size={17}/></button>;})}</section>}
 ...
 {confirm&&<div className={styles.backdrop}><div role="dialog" aria-modal="true" aria-labelledby="confirm-title" className={styles.dialog}><h2 id="confirm-title">{confirm==='delete'?'この記事を削除しますか？':'この内容をnoteに投稿します'}</h2><p>{draft.title}</p>{confirm==='publish'&&<><p>{draft.price?'有料 ¥'+draft.price.toLocaleString():'無料'} · {draft.scheduledAt?draft.scheduledAt.replace('T',' ')+'（日本時間）':'今すぐ公開'}</p><p>本文・価格・有料ラインを確認したうえで、ダンに投稿を依頼します。</p></>}<div className={styles.actions}><button onClick={()=>setConfirm(null)}>戻る</button><button className={styles.primary} disabled={busy||editing} onClick={()=>{if(confirm==='publish'){void send('publish');}else{void run(async()=>{await call('/dan-notion/blocks/'+selected!.id,'DELETE');setRows(v=>v.filter(x=>x.id!==selected!.id));setSelected(undefined);setDraft(emptyArticle());setDirty(false);setConfirm(null);});}}}>{confirm==='delete'?'削除する':'この内容で投稿を依頼'}</button></div></div></div>}
