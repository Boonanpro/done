...
まず一続きの原稿として読みやすく完成させる。素材に有料に足る独自の具体的な手順・判断材料がなければprice=0。ただし評価用の区切りとしてfree/paidへ分ける。文字数や比率で機械的に分割しない。有料にする場合も前後で同じ説明を繰り返さず、「有料部分では…」という予告は原則入れない。価格は提案であり公開操作はしない。
...
''' + transcript + '\n</素材>'
            ...
            prompt += '\neditorialNotesには区切りと画像の理由だけを短く記す。本文の構成を説明のために変更しない。'
