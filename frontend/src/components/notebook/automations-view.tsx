    ...
    <div className="flex h-full">
      <aside className="w-80 border-r bg-muted/20 flex flex-col">
        <div className="p-3 border-b space-y-2">
          ...
          <Input placeholder="新しい自動化の名前" value={newName} onChange={(e) => setNewName(e.target.value)} className="h-8 text-xs" />
          <select value={newKind} onChange={(e) => setNewKind(e.target.value as TriggerKind)} className="w-full h-8 text-xs border rounded px-2 bg-background">
            {KINDS.map((k) => <option key={k} value={k}>{k}</option>)}
          </select>
          <Button size="sm" className="w-full h-8" onClick={() => create.mutate()} disabled={create.isPending}>
          ...
          <div className="p-2 space-y-0.5">
            {list.map((t) => (
              <button
                ...
                className={`w-full flex items-center justify-between px-2 py-1.5 rounded text-left text-sm hover:bg-muted ${t.id === selected ? 'bg-muted font-medium' : ''}`}
              >
                <div className="min-w-0">
                  <div className="truncate">{t.name}</div>
                  <div className="text-[10px] text-muted-foreground">{t.kind} · 発火 {t.fire_count}</div>
                </div>
                {t.is_enabled ? <span className="text-[10px] text-green-600">ON</span> : <span className="text-[10px] text-muted-foreground">OFF</span>}
              </button>
            ))}
            {list.length === 0 && !triggers.isLoading && (
              <div className="text-xs text-muted-foreground px-2 py-4 text-center">まだ自動化がありません</div>
            )}
          </div>
        </ScrollArea>
      </aside>
          ...
          <>
            <div className="flex items-start justify-between">
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={() => fire.mutate(current.id)} disabled={fire.isPending}>
                  {fire.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
                  手動発火
                </Button>
                <Button size="sm" variant="ghost" onClick={() => { if (confirm('削除しますか？')) del.mutate(current.id); }}>
            ...
            <section>
              ...
              <pre className="text-xs bg-muted rounded p-3 overflow-auto">{JSON.stringify(current.actions, null, 2)}</pre>
            </section>
                ...
                {(runs.data?.runs ?? []).map((r) => (
                  <div key={r.id} className="text-xs border rounded p-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-muted-foreground">{new Date(r.started_at).toLocaleString('ja-JP')}</span>
                    </div>
                    {r.error && <div className="text-destructive mt-1">{r.error}</div>}
                  </div>
                ))}
                {(runs.data?.runs ?? []).length === 0 && <div className="text-xs text-muted-foreground">実行履歴なし</div>}
              </div>
            </section>
