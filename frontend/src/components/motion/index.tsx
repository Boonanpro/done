...
/** JS-pinned story section that does not rely on CSS position: sticky. */
export function PinnedStory({
  ...
  progressLabel = 'story',
  pinTop = 96,
  endHoldClassName = 'hidden h-[64vh] lg:block',
}: PinnedStoryProps) {
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      ...
      const maxY = Math.max(0, track.offsetHeight - media.offsetHeight);
      const nextY = Math.min(Math.max(pinTop - trackRect.top, 0), maxY);
      media.style.transform = `translateY(${nextY}px)`;

      const targetY = Math.min(window.innerHeight * 0.52, pinTop + media.offsetHeight * 0.52);
      let nextActive = 0;
      ...
      chapterRefs.current.forEach((chapter, index) => {
        ...
        const distance = Math.abs(chapterCenter - targetY);
  ...
  }, [chapters.length, pinTop]);

  return (
    <section className={`grid gap-12 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] ${className ?? ''}`}>
      <div ref={trackRef} className={`relative hidden lg:block ${mediaClassName ?? ''}`}>
        <div ref={mediaRef} className="absolute inset-x-0 top-0 h-[calc(100vh-8rem)] will-change-transform">
      ...
      <div className={`relative ${contentClassName ?? ''}`}>
        ...
        {chapters.map((chapter, index) => (
          <div
            ...
            className={`relative flex min-h-[72vh] flex-col justify-center py-16 pl-0 lg:min-h-[86vh] lg:pl-12 ${chapterClassName ?? ''}`}
          >
            <div
              aria-hidden="true"
