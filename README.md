...
- **Smart Fallback機能追加**: LLMを使って距離・コスト・利便性を考慮した最適な代替案を提案（全タスクタイプ対応）
...
| **Smart Fallback** | LLMで距離・コスト考慮した最適な代替案を提案（全タスクタイプ対応） | ✅ **新規追加（2024年12月27日）** |
...
| **電話タスク自動判定** | エージェントがPHONEタスクを自動判定 (10F) | ✅ **動作確認済み（2024年12月28日）** |
...
- 🤖 AIエージェント「ダン」が日本語で応答
- 📱 **電話タスク自動判定**: 「電話して」「電話で予約して」等のリクエストを自動認識し、AIが代わりに電話（予約・問い合わせ等）
...
- ✅ **タスクはSupabase DBに永続化されます**（2024年12月27日対応）
- サーバー再起動後もタスクは保持されます
- メモリキャッシュ + DB保存のハイブリッド方式で高速かつ永続的
...
| テスト | ディレクトリ | 目的 | 実行タイミング |
|-------|------------|------|--------------|
| **Unit Tests** | `tests/` | モック使用、高速テスト | 毎コミット（pre-commit） |
| **E2E Tests** | `tests_e2e/` | 実際のサービスを使用 | 手動・リリース前 |
...
E2Eテストは**実際のサービス**を使用します：
- ✅ 実際のSupabase DB（テスト用プロジェクト）
- ✅ 実際のClaude API
- ✅ 実際のPlaywrightブラウザ
- ✅ 実際の外部サービス（WILLER、Amazon等）

**セットアップ**:

```bash
# 1. テスト用環境ファイルを作成
cp env_test_example.txt .env.test

# 2. .env.test を編集してテスト用Supabaseプロジェクトを設定
#    ⚠️ 本番DBとは別のSupabaseプロジェクトを使用すること！
...
**注意**: E2Eテストは実際の外部サービスにアクセスするため：
- テスト用DBを使用すること（本番データ汚染防止）
- API利用料金が発生する可能性あり
- 外部サービスの状態により結果が変わる可能性あり
...
| 条件 | 更新するドキュメント |
|-----|-------------------|
| API追加/変更 | **README.md** の該当セクション |
| `feat:` `fix:` コミット | **CHANGELOG.md** の [Unreleased] セクション |
| 設計判断（代替案を検討した） | **docs/adr/** に新規ADR作成 |
...
### Added
- タスクのDB永続化 (Commits: `abc1234`, ADR: [001](docs/adr/001_xxx.md))
```

#### ADR作成の判断基準

- ✅ 作成する: 「なぜこうしたか」を将来忘れそう / 代替案を検討した
- ❌ 作成しない: 単純なバグ修正 / 小さなリファクタリング

ADRテンプレート: [docs/adr/README.md](docs/adr/README.md)

**重要なポイント：**
- 手動で成功したテストは必ず自動テストに組み込む
- 自動テストが通らないとコミットしない
- README.md / CHANGELOG.md / ADR は常に最新の状態を保つ
